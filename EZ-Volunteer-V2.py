...
def generate_report():
    ...
    newlab=ckt.CTkLabel(repwin,text="This will generate a TXT file containing a report of a volunteer's activities. \n It will NOT show the total amount of hours. ")
    newlab.pack(padx=10,pady=10)
    ...
    def entred():   
        ...
        except: newlab.configure(repwin,text=f"That volunteer doesn't exist")
        ...
        waa=fr'C:\Users\abhir\OneDrive\Education\Computer Science\Python\Projects\Volunteer System\{mudi}_report.txt'
        f = open(waa, "a")
        f.write(f"Volunteer report on {mudi}, requested by {newad}\n")

        for hh in range (len(dates)):
            f.write(f"\nVolunteered on {dates[hh]} from {starttimess[hh]} to {endtimess[hh]}, authorized by {admins[hh]}")
        f.write("\nGenerated by the EZ Volunteer System")
        ...
        viewin = ckt.CTk()
        viewin.geometry("720x480")
        viewin.title("Viewing Report - EZ Volunteer")
        newlab=ckt.CTkLabel(viewin,text=textstr)
        newlab.pack(padx=10,pady=10)
...
# Program written by Abhiram Vadali (24k-zilxh on GitHub)
# Python 3.12
# Made on Windows, IDK if it works on MacOS or Linux
